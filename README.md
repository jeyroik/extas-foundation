...
Данный пакет содержит базовые сущности Extas:
...
```
- встроенные события:
  - создание сущности `<сущность>.created`, срабатывает при сохранении нового экзепляра сущности в хранилище;
...
Ниже, после рассмотрения каждой базовой сущности, представлен рабочий пример использования всех сущностей вменьше и по отдельности.
...
### Пример использования каждой сущности по отделньости и вместе
...
class MyGetMutatedName extends Extension implements IGetMutatedName
{
    ...
    /**
     * Последним аргументов любого метода, который является расширением, передаётся экземпляр сущности, которая расширяется
     * @param null|My $my
     * 
     * @return string
     */
